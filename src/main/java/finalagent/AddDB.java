...
public class AddDB {
    ...
    public static void addKit(double latitude, double longitude) {
        try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD)) {
            try (PreparedStatement check = conn.prepareStatement(
                    "SELECT id FROM kit WHERE currentLatitude = ? AND currentLongitude = ?")) {
    ...
    public Double[] actualKitLocation(int kitId) {
        String sql = "SELECT currentLatitude, currentLongitude FROM Kit WHERE id = ?";
        try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD);
            ...
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                BigDecimal lat = rs.getBigDecimal("currentLatitude");
                BigDecimal lon = rs.getBigDecimal("currentLongitude");
 ...
 SELECT latitude, longitude FROM area  WHERE id = ?  """)){
    ...
    """;
        ...
        try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD);
            ...
            // Conversion des coordonnées en BigDecimal avec précision exacte
